...
public class MissionToMarsSystem {
    ...
    public void editMission() {
        ...
        if (missionExist) {
            ...
            while (edit) {
                ...
                switch (option2) {
                    ...
                    case 12:
                        ...
                        break;
                    default:
                        System.out.println("Your option is invalid. Please input again");
                        break;
                }
                int input = 0;
                do {
            ...
            String missionInfoEdit = missionEdit.getMissionId() + ";" + missionEdit.getMissionName() + ";" + missionEdit.getMissionDescription() +
                    ...
                    + missionEdit.getDestination() + ";" + missionEdit.getDestination() + ";" + missionEdit.getMissionStatus()
    ...
    public void createCandidateProfile()
    {
        ...
        do {
            ...
            id = UserInterface.readUserIntInput();
        } while (id == 0);
        ...
        String[] records = readFile("records.txt").split("\\n");
        String healthCri = "";
        for (int index = 0; index < records.length; index++) {
            String[] item = records[index].split(";");
            if (item[0].equals(user.getUserID())) {
                healthCri = item[1] + ";" + item[2];
            }
        }
    ...
    public void editCandidateProfile()
    {
        ...
        String canNewInfo = "";
        for (int index = 0; index < candidateInfo.length; index++) {
            canNewInfo = canNewInfo + canList.get(index).getUserID() + ";" + canList.get(index).getCanName() + ";" +
                    canList.get(index).getBirthday() + ";" + canList.get(index).getAddress() + ";" +
                    canList.get(index).getNationality() + ";" + canList.get(index).getIdentificationNumb() + ";" +
                    canList.get(index).getGender() + ";" + canList.get(index).getAllergies() + ";" +
                    canList.get(index).getFoodPreference() + ";" + canList.get(index).getQualification() + ";" +
                    canList.get(index).getWorkExperience() + ";" + canList.get(index).getOccupation() + ";" +
                    canList.get(index).getComputerSkill() + ";" + canList.get(index).getLanguage() + ";" +
                    canList.get(index).isHealthRecord() + ";" + canList.get(index).isCriminalRecord() + "\n";
        }
        ...
        do {
            ...
            input = UserInterface.readUserIntInput();
            if (input == 1) {
               overwriteFile("CandidateInfo.txt", canNewInfo);
                System.out.println("Successfully submitted. ");
            } else if (input == 2)
                System.out.println("The information is not submitted. ");
            else
