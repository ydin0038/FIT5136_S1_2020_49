...
public class UserInterface {
    ...
    public static void homeMenuforCandidate() {
        while (true) {
            ...
            switch (i) {
                case 1:
                    String[] canFile = system.readFile("CandidateInfo.txt").split("\\n");
                    int flag = 0;
                    for (int index = 0; index < canFile.length; index++) {
                        String[] item = canFile[index].split(";");
                        if (item[0].equals(system.getUser().getUserID())) {
                            System.out.println("You have created your profile. You can't create it again.");
                            flag = 1;
                        }
                    }
                    if ( flag == 0 )
                        ...
                        break;
                case 2:
                    String[] canFile2 = system.readFile("CandidateInfo.txt").split("\\n");
                    int flag2 = 0;
                    for (int index = 0; index < canFile2.length; index++) {
                        String[] item = canFile2[index].split(";");
                        if (item[0].equals(system.getUser().getUserID())) {
                    ...
                    if ( flag2 == 0 )
                        System.out.println("You haven't created your profile. You should create it first.");
                        break;
                case 3:
                    logOut();
                    break;
